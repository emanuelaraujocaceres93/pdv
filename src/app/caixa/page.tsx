      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        ...
        <div className="space-y-2 max-h-[400px] overflow-y-auto">
          {vendas.length === 0 ? <p className="text-gray-400 text-center py-4">Nenhuma venda registrada</p> : vendas.map(v => (
            <div key={v.id} className="p-4 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100 transition" onClick={() => { setModalDetalhes(v); carregarItensVenda(v.id) }}>
              <div className="flex justify-between items-center">
                <div><span className="font-bold text-green-600 text-lg">R$ {v.total.toFixed(2)}</span><div className="text-xs text-gray-500 mt-1">{new Date(v.created_at).toLocaleString()}</div></div>
      ...
      {modalDetalhes && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4"><h2 className="text-2xl font-bold text-amber-800">🧾 Detalhes da Venda</h2><button onClick={() => setModalDetalhes(null)} className="text-gray-500 text-3xl">&times;</button></div>
            ...
            <div className="space-y-2 mb-4">{itensVenda.map(item => (<div key={item.id} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg"><div><p className="font-medium">{item.products?.name || 'Produto'}</p><p className="text-sm text-gray-500">{item.quantity}x R$ {item.price.toFixed(2)}</p></div><p className="font-semibold text-green-600">R$ {item.total.toFixed(2)}</p></div>))}</div>
