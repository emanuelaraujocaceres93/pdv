...
export default function ComandasPage() {
  ...
  return (
    <div>
      ...
      {comandas.length === 0 ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {comandas.map((comanda) => (
            <div key={comanda.id} className="bg-white rounded-lg shadow-md p-6 cursor-pointer hover:shadow-lg" onClick={() => { setModalProdutos(comanda); carregarItens(comanda.id) }}>
              <div className="flex justify-between items-start">
                <div><h3 className="text-xl font-semibold text-amber-800">{comanda.table_number}</h3><p className="text-sm text-gray-500">{new Date(comanda.created_at).toLocaleString()}</p></div>
      ...
      {/* Modal Produtos da Comanda - LADO A LADO em todas as telas */}
      {modalProdutos && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto p-4">
          <div className="bg-white rounded-xl p-6 w-full max-w-6xl max-h-[90vh] overflow-y-auto">
            ...
            {/* LADO A LADO - flex row em todas as telas */}
            <div className="flex flex-row gap-6">
              ...
              {/* Itens da Comanda - 40% - SEMPRE DO LADO DIREITO */}
              <div className="w-[40%]">
                  ...
                  {itens.length === 0 ? <p className="text-gray-400 text-center py-8">Nenhum item adicionado</p> : itens.map(item => (
                    <div key={item.id} className="flex justify-between items-center bg-gray-50 rounded-lg p-3">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-500">{item.quantity}x R$ {item.price.toFixed(2)} = <span className="font-semibold text-green-600">R$ {(item.price * item.quantity).toFixed(2)}</span></p>
                      </div>
                      <button onClick={() => removerItem(item.id, modalProdutos.id)} className="text-red-500 ml-2">Remover</button>
                    </div>
                  ))}
                </div>
                <div className="mt-6 pt-4 border-t">
                  ...
                  <button onClick={() => setModalPagamento(true)} className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 rounded-lg text-lg">
                    💰 Fechar Comanda
                  </button>
                </div>
              </div>
