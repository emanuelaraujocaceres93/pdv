  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4 md:mb-6">
        <h1 className="text-2xl md:text-3xl font-bold text-amber-800">📋 Comandas</h1>
          ...
          {comandas.map((comanda) => (
            <div key={comanda.id} className="bg-white rounded-lg shadow-md p-4 md:p-6 cursor-pointer hover:shadow-lg transition" onClick={() => { setModalProdutos(comanda); carregarItens(comanda.id) }}>
              <div className="flex justify-between items-start">
                <div><h3 className="text-base md:text-xl font-semibold text-amber-800">{comanda.table_number}</h3><p className="text-xs md:text-sm text-gray-500">{new Date(comanda.created_at).toLocaleString()}</p></div>
                <span className="px-2 py-1 rounded text-xs bg-green-100 text-green-700">Aberta</span>
              </div>
      ...
      {modalComanda && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-5 md:p-6 w-full max-w-md"><h2 className="text-xl font-bold text-amber-800 mb-4">Nova Comanda</h2>
            ...
            <div className="flex gap-3"><button onClick={criarComanda} className="flex-1 bg-amber-700 text-white py-2 rounded-lg">Criar</button><button onClick={() => setModalComanda(false)} className="flex-1 bg-gray-300 py-2 rounded-lg">Cancelar</button></div>
          </div>
        </div>
      )}
      ...
      {modalProdutos && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto p-4">
          <div className="bg-white rounded-xl p-4 md:p-6 w-full max-w-6xl max-h-[95vh] overflow-y-auto">
            ...
            <div className="flex flex-row gap-4 md:gap-6">
              ...
              {/* Coluna de Produtos - 60% */}
              <div className="w-[60%]">
                <h3 className="font-bold text-sm md:text-lg mb-2 md:mb-4">📦 Adicionar Produtos</h3>
                <div className="space-y-2 md:space-y-3 max-h-[calc(100vh-250px)] overflow-y-auto pr-2">
                  {produtos.map(p => (
                    <div key={p.id} className="flex justify-between items-center border-b py-2 md:py-3">
                      <div><p className="font-medium text-xs md:text-sm">{p.name}</p><p className="text-green-600 font-semibold text-xs md:text-sm">R$ {p.price.toFixed(2)}</p></div>
                      <button onClick={() => adicionarProduto(modalProdutos.id, p)} className="bg-amber-700 hover:bg-amber-800 text-white px-3 py-1 md:px-4 md:py-2 rounded-lg text-xs md:text-sm">+ Adicionar</button>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Coluna de Itens - 40% */}
              <div className="w-[40%]">
                <h3 className="font-bold text-sm md:text-lg mb-2 md:mb-4">🛒 Itens da Comanda</h3>
                <div className="space-y-2 md:space-y-3 max-h-[calc(100vh-250px)] overflow-y-auto pr-2">
                  {itens.length === 0 ? <p className="text-gray-400 text-center py-6 md:py-8 text-sm">Nenhum item adicionado</p> : itens.map(item => (
                    <div key={item.id} className="flex justify-between items-center bg-gray-50 rounded-lg p-2 md:p-3">
                      <div><p className="font-medium text-xs md:text-sm">{item.products.name}</p><p className="text-[10px] md:text-xs text-gray-500">{item.quantity}x R$ {item.price.toFixed(2)} = <span className="font-semibold text-green-600">R$ {(item.price * item.quantity).toFixed(2)}</span></p></div>
                      <button onClick={() => removerItem(item.id, modalProdutos.id)} className="text-red-500 text-xs md:text-sm">Remover</button>
                    </div>
                  ))}
                </div>
                <div className="mt-4 md:mt-6 pt-3 md:pt-4 border-t"><div className="flex justify-between items-center mb-3 md:mb-4"><span className="text-base md:text-xl font-bold">Total:</span><span className="text-lg md:text-2xl font-bold text-green-600">R$ {totalItens.toFixed(2)}</span></div><button onClick={() => setModalPagamento(true)} className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-2 md:py-3 rounded-lg text-sm md:text-lg">💰 Fechar Comanda</button></div>
              </div>
            </div>
          </div>
        </div>
      ...
      {modalPagamento && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-5 md:p-6 w-full max-w-md"><div className="text-center mb-4 md:mb-6"><div className="text-4xl md:text-5xl mb-2 md:mb-3">💰</div><h2 className="text-xl md:text-2xl font-bold">Forma de Pagamento</h2><p className="text-gray-500 mt-1">Total: <span className="font-bold text-green-600">R$ {totalItens.toFixed(2)}</span></p></div>
            <div className="space-y-2 md:space-y-3">
              <button onClick={() => setPagamentoSelecionado('dinheiro')} className={'w-full p-3 md:p-4 rounded-xl border-2 text-left flex items-center gap-3 ' + (pagamentoSelecionado === 'dinheiro' ? 'border-green-500 bg-green-50' : 'border-gray-200')}><span className="text-xl md:text-2xl">💵</span><span>Dinheiro</span>{pagamentoSelecionado === 'dinheiro' && <span className="ml-auto text-green-500">✓</span>}</button>
              <button onClick={() => setPagamentoSelecionado('cartao_credito')} className={'w-full p-3 md:p-4 rounded-xl border-2 text-left flex items-center gap-3 ' + (pagamentoSelecionado === 'cartao_credito' ? 'border-green-500 bg-green-50' : 'border-gray-200')}><span className="text-xl md:text-2xl">💳</span><span>Cartão Crédito</span>{pagamentoSelecionado === 'cartao_credito' && <span className="ml-auto text-green-500">✓</span>}</button>
              <button onClick={() => setPagamentoSelecionado('cartao_debito')} className={'w-full p-3 md:p-4 rounded-xl border-2 text-left flex items-center gap-3 ' + (pagamentoSelecionado === 'cartao_debito' ? 'border-green-500 bg-green-50' : 'border-gray-200')}><span className="text-xl md:text-2xl">💳</span><span>Cartão Débito</span>{pagamentoSelecionado === 'cartao_debito' && <span className="ml-auto text-green-500">✓</span>}</button>
              <button onClick={() => setPagamentoSelecionado('pix')} className={'w-full p-3 md:p-4 rounded-xl border-2 text-left flex items-center gap-3 ' + (pagamentoSelecionado === 'pix' ? 'border-green-500 bg-green-50' : 'border-gray-200')}><span className="text-xl md:text-2xl">📱</span><span>PIX</span>{pagamentoSelecionado === 'pix' && <span className="ml-auto text-green-500">✓</span>}</button>
            </div>
